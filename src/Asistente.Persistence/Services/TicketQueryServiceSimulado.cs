...
///
/// Filtra por nombre de usuario igual que el adaptador real, de modo que el comportamiento
/// sea el mismo: cada quien ve solo sus tickets. Para cambiar a la base real basta poner
/// Provider = "Oracle" en la sección Tickets.
/// </summary>
public sealed class TicketQueryServiceSimulado : ITicketQueryService
{
        ...
        string usuario, string? termino, int maximo, CancellationToken ct)
    {
        ...
        // Solo los clientes que aparecen en tickets de este usuario: listar los demás
        // filtraría a nada y revelaría con qué clientes trabaja el resto del equipo.
        var propios = Tickets.Where(x => EsDe(x, usuario)).Select(x => x.ClienteId).ToHashSet();

        IReadOnlyList<ClienteDto> resultado = Clientes
            .Where(c => propios.Contains(c.Id))
            .Where(c => t.Length == 0
    ...
    public Task<TicketDto?> ObtenerPorIdAsync(
    ...
    /// <summary>El vínculo entre ambos sistemas es el nombre de usuario.</summary>
    private static bool EsDe(TicketDto ticket, string usuario) =>
        string.Equals(ticket.AsignadoA, usuario, StringComparison.OrdinalIgnoreCase);
